        ...
        if os.path.exists("account_info.txt"):
            with open("account_info.txt", "r") as file:
                for line in file:
                    ...
                    all_accounts[name.strip()] = balance.strip()

        # 현재 계좌 정보 업데이트 또는 추가
        all_accounts[self.name.strip()] = str(self.balance)
        ...
        if os.path.exists("account_info.txt"):
            with open("account_info.txt", "r") as file:
                for line in file:
                    ...
                    if account_name.strip() == name.strip():
                        print(f"{name}님의 기존 계좌 정보를 불러왔습니다. 잔액: {balance}원")
                        return BankAccount(account_name.strip(), int(balance.strip()))
...
while True:
    ...
    match choice:
        ...
        case "4":
            ...
            print(f"{account.name}님의 계좌를 종료합니다. 최종 잔액은 {account.balance}원입니다.")
            break
...
#                 for line in file:
#                     name, balance = line.strip().split(",")
#                     all_account[name.strip()] = balance.strip()
...
#         # 모든 정보를 다시 파일에 저장
#         with open("account_info.txt", "w") as file:
#             for name, balance in all_account.items():
#                 file.write(f"{self.name}, {self.balance}\n")
